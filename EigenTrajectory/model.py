...
class EigenTrajectory(nn.Module):
    ...
    def calculate_parameters_time_test(self, obs_traj, pred_traj, cycle_number, svd_method='torch'):
        ...
        data_m = self.ET_m_descriptor.time_test(obs_m_traj, pred_m_traj,
                                                cycle_number=cycle_number,
                                                svd_method=svd_method)
        data_s = self.ET_s_descriptor.time_test(obs_s_traj, pred_s_traj,
                                                cycle_number=cycle_number,
                                                svd_method=svd_method)

        print(f'matrix {pred_m_traj.shape} with time: {data_m}')
        print(f'matrix {pred_s_traj.shape} with time: {data_s}')

        return True

    def calculate_parameters(self, obs_traj, pred_traj, svd_method='torch'):
        r"""Calculate the ET descriptors of the EigenTrajectory model
        ...
        # Descriptor initialization
        data_m = self.ET_m_descriptor.parameter_initialization(obs_m_traj, pred_m_traj, svd_method=svd_method)
        data_s = self.ET_s_descriptor.parameter_initialization(obs_s_traj, pred_s_traj, svd_method=svd_method)
